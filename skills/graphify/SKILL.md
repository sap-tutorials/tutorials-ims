...
> **Attribution:** graphify wraps the third-party `graphifyy` Python package by
> [@safishamsi](https://github.com/safishamsi). This SKILL.md is a driver for that
> tool; the pipeline logic and package are the author's work. If graphify saves
> you time, support it: https://github.com/sponsors/safishamsi
>
> **Platform note:** the commands below use PowerShell (Windows). On macOS/Linux,
> translate the `python -c "..."` blocks to your shell (the Python is identical;
> only the shell quoting and `Remove-Item`/`Out-File` idioms differ).
