...
describe('track-colors', () => {
  it('assigns a stable colour per track regardless of input order/dupes', () => {
    const a = buildTrackColorMap([{name:'CAP'},{name:'ABAP'},{name:'AI'},{name:'ABAP'}]);
    const b = buildTrackColorMap([{name:'AI'},{name:'ABAP'},{name:'CAP'}]);
  ...
  it('every colour has bg/border/text strings', () => {
    const m = buildTrackColorMap([{name:'X'}]);
    const c = m.get('X')!;
