...
describe('RESOURCE_TYPE_CONFIG — registry shape', () => {
  it('is an array with exactly 9 entries', () => {
    expect(Array.isArray(RESOURCE_TYPE_CONFIG)).toBe(true);
    expect(RESOURCE_TYPE_CONFIG).toHaveLength(9);
  });
  ...
  it('type values are exactly the nine expected external types in priority order', () => {
    const types = RESOURCE_TYPE_CONFIG.map((e) => e.type);
