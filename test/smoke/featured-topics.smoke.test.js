...
describe.skipIf(!SRV)('GET /homepage/featuredTopics() smoke (#1032)', () => {
  it('returns 200 JSON with snapshot and etag', async () => {
    const res = await fetchWithRetry(`${SRV}/homepage/featuredTopics()`);
  ...
  it('returns 304 on If-None-Match round-trip', async () => {
    const first = await fetchWithRetry(`${SRV}/homepage/featuredTopics()`);
    expect(first.status).toBe(200);
    const etag = first.headers.get('etag');
    expect(etag).toBeTruthy();

    const second = await fetchWithRetry(`${SRV}/homepage/featuredTopics()`, {
