...
- **Observability metrics module** (`srv/lib/metrics.js`, #805) — In-memory counters/gauges/reservoirs, drained every 5 minutes by `srv/jobs/metrics-rollup-job.js` into `MetricSnapshots` rows. Two env flags: `METRICS_ENABLED` (default `true`; set `false` as a kill-switch) and `METRICS_DB_WRAP` (default `false`; when `true`, [srv/lib/metrics-db-wrap.js](srv/lib/metrics-db-wrap.js) installs the passive `cds.db.run` / `cds.db.tx` wrapper at `cds.on('served')` time to emit `db.acquire.ms`, `db.tx.ms`, `db.tx.run.ms`, `db.pool.timeout` — see [docs/developers/architecture/observability.md § DB-wrapper rollout](docs/developers/architecture/observability.md)). The rollup job does NOT use `job-lock` — both CF instances write per-instance rows under composite primary key `(windowStart, metric, instanceId)`. The retention jobs (30d/90d) DO use `job-lock`. Live snapshot at `/admin-ui/#metrics` (30s polling), `GET /admin/getMetricsSnapshot()` (CAP function, Admin scope), and `GET /admin/metrics/live` (Express, Admin scope required — handler checks `user.is('Admin')` explicitly since `basicAuthMiddleware` is pass-through). See [docs/developers/architecture/observability.md](docs/developers/architecture/observability.md).
