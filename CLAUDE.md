...
- **TechEd sessions subsystem (#2312)** — new `TechEd*` entities (`db/external/teched.cds`, NOT reused `CommunityEvents`), loaded via a **`using` side-effect** in `db/external-content.cds` (don't delete the import); RainFocus ingest (`POST /api/search`, per-venue `rfWidgetId`/`rfApiProfileId` from `TECHED_RF_{TE26,TEV26}_*` env, IDs may rotate); idempotent seed via `scripts/seed-teched.cjs`; public `/build/teched` feed; `/teched/` page + island, cron jobs, KG projection, embeddings, and Devtoberfest cross-linking are later units. → [teched.md](docs/developers/reference/teched.md).
