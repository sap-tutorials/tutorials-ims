...
- **`@cap-js/data-privacy` deferred, annotations shipped (#960)** — Plugin install rolled back at 0.6.2 due to two crashes in `cds build --production` (per-task builds don't include plugin's own aspects; aggregate views atop `DataSubjectDetails` lose the DataSubjectID via GROUP BY). Annotation cleanups landed anyway (Concepts→`@cds.changelog`, 7 entities upgraded to `DataSubjectDetails+delete`, 3 kept `Other+DataSubjectRole:Developer`). When retrying plugin adoption: verify `cds build --production` succeeds against our schema FIRST; pick up Tasks 7/8/9 blueprints from the plan; do NOT re-annotate BranchDecisions as `DataSubjectDetails` (aggregate views make it plugin-incompatible). See [authentication.md § Data Privacy Integration](docs/developers/architecture/authentication.md), spec at [docs/superpowers/specs/2026-07-04-960-data-privacy-plugin-design.md](docs/superpowers/specs/2026-07-04-960-data-privacy-plugin-design.md).
