sap.ui.define([
  ...
  // explainer (small prompts + small responses). Same constant as the
  // verb-definitions + shelf-definitions controllers — update all three if
  // costs shift. The homepage list scale (~60 rows) makes the bulk estimate
  // more visible to admins (~$0.30 for a full fill-blanks vs ~$0.03 for verbs).
  ...
  const LR_ENTITY_PATH = "/HomepageShelves";
  ...
  async function countBlankRows() {
    const res = await fetch('/admin/HomepageShelves?$filter=authoringStatus%20eq%20%27BLANK%27&$count=true&$top=0', {
      credentials: 'include',
    });
    if (!res.ok) return 0;
  ...
  return {
        ...
        MessageBox.warning(
          `Generate AI explainers for ${n} blank row${n === 1 ? '' : 's'}? Estimated cost: ${fmtUsd(estCents)}. This will not overwrite AI-seeded or human-reviewed rows.`,
          {
            ...
            onClose: async (action) => {
              ...
              try {
                const result = await postAdminAction("generateShelfEntryExplainers", { ids: [], mode: "fill-blanks" });
    ...
    onRegenerateSelected: async function (oEvent) {
      ...
      MessageBox.warning(msg, {
        ...
        onClose: async (action) => {
          ...
          try {
            const result = await postAdminAction("generateShelfEntryExplainers", { ids, mode: "regenerate-selected" });
    ...
    onMarkReviewedSelected: async function (oEvent) {
      ...
      try {
        const result = await postAdminAction("bulkMarkShelfEntryExplainerReviewed", { ids });
    ...
    onRegenerateOne: async function (oEvent) {
      ...
      MessageBox.warning(msg, {
        ...
        onClose: async (action) => {
          ...
          MessageToast.show("Regenerating…");
          try {
            const result = await postAdminAction("generateShelfEntryExplainers", { ids: [row.ID], mode: "regenerate-selected" });
    ...
    onMarkReviewed: async function (oEvent) {
      ...
      const row = ctx.getObject();
      try {
        // Calls the dedicated markShelfEntryExplainerReviewed action (added in
        // PR 3b Task 7). Plain OData PATCH would be rejected because
        // authoringStatus is @Common.FieldControl: #ReadOnly.
        await postAdminAction("markShelfEntryExplainerReviewed", { id: row.ID });
