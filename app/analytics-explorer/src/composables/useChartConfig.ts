...
import type { DimensionConfig, MeasureConfig, FilterConfig } from '../api/odata'

// ── Re-export shared types so consumers can import them from here ────────────
export type { DimensionConfig, MeasureConfig, FilterConfig }
...
export function suggestChartType(
  ...
  if (dimCount === 1 && mCount === 1) {
    const dc = (dimensions[0] as DimensionConfig & { distinctCount?: number }).distinctCount
