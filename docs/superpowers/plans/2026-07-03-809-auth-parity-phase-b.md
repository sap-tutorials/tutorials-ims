...
- [ ] **Step B3.1: AdminService**
  ...
  - `describe('AdminService — entities (auth matrix)', ...)` — one `matrix()` per top-level entity (Users, Tutorials, Missions, Groups, Events, Prizes, Tags, Advocates, Alerts, HomepageShelves, HomepageConfig, VerbDefinitions, ShelfDefinitions, ChatSettings, KnowledgeGraphSettings, JobControls, DevtoberfestConfig, Secrets, and the code-list / analytics/readonly ones abbreviated as a group).
  - `describe('AdminService — free-standing actions/functions', ...)` — one `matrix()` per top-level action from the recon table.
...
| `feedback.auth.test.js` | POST /feedback/submit | Anonymous by design. Assert: no bearer required, salt env present required (503 if missing). |
| `health.auth.test.js` | GET /health, /health/db, /health/auth | /health and /health/db anonymous. /health/auth requires authenticated (401 anonymous). |
...
| `build-endpoints.auth.test.js` | All `/build/*` GETs | Anonymous by design (except POST /build/repo-catalog which needs bearer). Assert. |
