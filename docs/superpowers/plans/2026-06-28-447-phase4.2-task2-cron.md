...
describe('fetch-blog-posts-job — merge-on-write (#707) + crash-safety (#708)', () => {
  ...
  it('respects budget gate: stops after N extractions; budgetExhausted=true', async () => {
    const { BlogPosts, ChatSettings } = cds.entities('com.sap.developers.ims.external');
    await INSERT.into(BlogPosts).entries({
      slug: 'bp-99999', title: 'Seed', url: 'u', khorosMessageId: '99999',
      postedAt: '2026-04-01T00:00:00.000Z', sourceId: '99999',
      contentHash: 'X', lastExtractedHash: 'X',
    ...
    // Khoros returns 3 new posts; budget = 2.
    _setMockTransport({
      async call() {
        return {
          status: 'success',
    ...
    const { ChatSettings: Cs } = cds.entities('com.sap.developers.ims');
    // Set the per-day budget to 2.
    await UPSERT.into(Cs).entries({ ID: 'singleton', blogPostExtractBudgetPerDay: 2 });
...
export async function runFetchBlogPosts(deps = {}) {
  ...
  const sinceIso = maxPosted instanceof Date ? maxPosted.toISOString() : maxPosted;
...
- [ ] **Step 10: Add `blogPostExtractBudgetPerDay` column to `ChatSettings`**

The cron reads `ChatSettings.blogPostExtractBudgetPerDay`. The column needs to exist or the SELECT throws (which the cron handles, but unit tests assume the column exists for the budget-gate test).

```bash
grep -n "learningJourneyExtractBudgetPerDay" db/ -r
```

Find the file declaring `learningJourneyExtractBudgetPerDay` (likely `db/schema-ext.cds` or `db/admin.cds`). Add a sibling field:

```cds
extend entity ChatSettings with {
  blogPostExtractBudgetPerDay : Integer default 50;
}
```

Or inside the entity definition if it lives there. Re-run `npx cds build --production` and stage the regenerated `db/last-dev/csn.json`.

- [ ] **Step 11: Run cron tests; expect 5/5 PASS**
...
- [ ] **Step 12: Register the cron in `srv/jobs/scheduler.js`**
...
- [ ] **Step 14: Extend `srv/lib/kg-projection.js` with `buildBlogPostTriples(...)`**
...
- [ ] **Step 15: Wire it into the main projection loop**
...
- [ ] **Step 16: Extend `srv/lib/published-concepts-query.js` with `blogPosts[]`**

```bash
grep -n "learningJourneys" srv/lib/published-concepts-query.js
```

Find where `learningJourneys` is attached per concept. Add a parallel block:
...
- [ ] **Step 17: Write a unit test `test/unit/srv/published-concepts-query-with-blog-posts.test.js`**
...
- [ ] **Step 19: Commit projection + payload extension**
...
- [ ] **Step 20: Extend `srv/knowledge-graph-service.js` neighborhood `otherResources`**

```bash
grep -n "otherResources\|learning-journey" srv/knowledge-graph-service.js
```

Find where Phase 4.1 populates `otherResources` with journey rows. Add a parallel query for blog posts and merge the two sources, sorting by overlap, capping at top-5 TOTAL across types per spec §9:

```javascript
// Phase 4.2 (#447): widen otherResources to UNION across journeys + blog posts.
const { BlogPosts, BlogPostConceptLinks } = cds.entities('com.sap.developers.ims.external');

const blogOverlaps = teachesConceptIds.length === 0
  ? []
  : await SELECT.from(BlogPostConceptLinks)
      .columns('post_ID', 'count(*) as overlap')
      .where({ concept_ID: { in: teachesConceptIds } })
      .groupBy('post_ID');

const blogRows = blogOverlaps.length === 0
  ? []
  : await SELECT.from(BlogPosts)
      .columns('ID', 'slug', 'title', 'url', 'authorName', 'postedAt')
      .where({ ID: { in: blogOverlaps.map(b => b.post_ID) } });

const overlapByBlogPostId = new Map(blogOverlaps.map(b => [b.post_ID, b.overlap]));

const blogOtherResources = blogRows.map(b => ({
  type: 'blog-post',
  slug: b.slug,
  title: b.title,
  url: b.url,
  ...
  postedAt: b.postedAt,
  overlapCount: overlapByBlogPostId.get(b.ID) ?? 0,
}));

// Merge journey + blog rows; sort by overlap desc; cap top-5 TOTAL.
const combined = [...journeyOtherResources, ...blogOtherResources]
  .sort((a, b) => (b.overlapCount ?? 0) - (a.overlapCount ?? 0))
  .slice(0, 5);

result.otherResources = combined;
```

Adapt to the file's actual variable names — `journeyOtherResources` may be named differently. Read it first.

- [ ] **Step 21: Extend the existing neighborhood-result-shape test or add a new one**
...
- [ ] **Step 22: Run neighborhood tests; expect PASS**
...
- [ ] **Step 24: Create `scripts/seed-blog-posts.cjs`**
...
(async () => {
  ...
  // For COMMIT mode, we directly use runFetchBlogPosts with the wide window
  // because it already handles upsert + extract + persist. The
  // ChatSettings.blogPostExtractBudgetPerDay gate is bypassed by setting
  // the budget HIGH for this run via env var override (cron reads ChatSettings
  // first; we monkey-patch by temporarily setting it).
  //
  // SAFER ALTERNATIVE: run a separate budgetless loop here that re-implements
  // the upsert+extract chain. For 4.2 PR-2, we go with the simpler approach:
  // raise the ChatSettings budget temporarily, run the cron, restore it.

  const { ChatSettings } = cds.entities('com.sap.developers.ims');
  const original = await SELECT.one.from(ChatSettings).columns('blogPostExtractBudgetPerDay');
  await UPSERT.into(ChatSettings).entries({ ID: 'singleton', blogPostExtractBudgetPerDay: LIMIT });

  try {
    // Override the cron's MAX(postedAt) sinceIso by NOT pre-seeding —
    // but the cron aborts on empty BlogPosts. So we seed a single sentinel row
    // first if the table is empty:
    const existing = await SELECT.one.from(BlogPosts).columns('ID').limit(1);
    if (!existing && !RESUME) {
      // Seed a sentinel at sinceIso so the cron's MAX(postedAt) returns sinceIso.
      await INSERT.into(BlogPosts).entries({
        slug: 'bp-seed-sentinel',
        title: '(seed sentinel — operator-inserted by seed-blog-posts.cjs)',
        url: 'https://example.com/seed',
        khorosMessageId: 'seed-sentinel',
        postedAt: sinceIso,
        sourceId: 'seed-sentinel',
        contentHash: 'SEED',
        lastExtractedHash: 'SEED',
      });
      console.log(`[seed] inserted sentinel row at postedAt=${sinceIso}`);
    }
    ...
    await UPSERT.into(ChatSettings).entries({
      ID: 'singleton',
      blogPostExtractBudgetPerDay: original?.blogPostExtractBudgetPerDay ?? 50,
    });
  }
...
> **NOTE for the implementer:** the sentinel-row approach for first-run bootstrap is ONE way to honor the "cron refuses to self-bootstrap" rule from spec §7 step 4. An alternative is to refactor `runFetchBlogPosts` to accept an `overrideSinceIso` parameter for backfill paths. If that refactor is small (~10 lines), prefer it — it's cleaner. Mention which approach was taken in the commit message.

- [ ] **Step 25: Add `.migration-data/` to gitignore if not already**

```bash
grep -q '^.migration-data/' .gitignore || echo '.migration-data/' >> .gitignore
```

- [ ] **Step 26: Commit backfill script**
...
- [ ] **Step 27: Create `test/hybrid/blog-posts-cron.test.js`**
...
- [ ] **Step 28: Commit hybrid test + push**
