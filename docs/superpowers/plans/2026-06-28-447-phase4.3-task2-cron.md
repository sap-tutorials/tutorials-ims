...
```json
{
  ...
  "tokenUsage": { "prompt": 3000, "completion": 420 }
}
```
...
```bash
grep -n "cron.schedule\|runWithLock\|fetch-blog-posts" srv/jobs/scheduler.js
```

Expected: Phase 4.1's journey cron + Phase 4.2's blog cron are registered as siblings, each using `runWithLock(name, ttlMs, async () => {...})` inside `cron.schedule(expr, async () => {...})`. Confirm the actual shape before pattern-matching below — adapt indentation, import style, and `runWithLock` TTL to match.

Add a sibling registration for the Phase 4.3 weekly cron:
...
Verify Phase 4.2's `buildBlogPostTriples` signature first — it takes ARRAYS of all rows + all links and iterates internally:

```bash
grep -n "buildBlogPostTriples\|buildLearningJourneyTriples" srv/lib/kg-projection.js
```

Expected: `export function buildBlogPostTriples({ posts = [], links = [] } = {})` at line ~436. Mirror that signature exactly:

```javascript
/**
 * Phase 4.3 (#447): emit RDF triples for an array of DiscoveryMission rows.
 *
 * Mirrors buildBlogPostTriples — takes arrays of all missions + all links,
 * iterates internally. Per-mission TTL gate via isWithinTTL('discovery-mission',
 * mission.lastSeenAt). Emits :rdf:type, :title, :slug, :effortLevel,
 * :categorySlug, and :teaches for each link.
 */
export function buildDiscoveryMissionTriples({ missions = [], links = [] } = {}) {
  const triples = [];

  // Index links by mission slug for O(1) lookup per row.
  const linksByMissionSlug = new Map();
  for (const l of links) {
    if (!l.missionSlug || !l.conceptSlug) continue;
    if (!linksByMissionSlug.has(l.missionSlug)) linksByMissionSlug.set(l.missionSlug, []);
    linksByMissionSlug.get(l.missionSlug).push(l);
  }

  for (const m of missions) {
    if (!isWithinTTL('discovery-mission', m.lastSeenAt)) continue;

    const iri = iriDiscoveryMission(m.slug);
    triples.push([iri, 'rdf:type', 'kg:DiscoveryMission']);
    triples.push([iri, 'kg:title', JSON.stringify(m.title)]);
    triples.push([iri, 'kg:slug', JSON.stringify(m.slug)]);
    if (m.effortLevel != null) {
      triples.push([iri, 'kg:effortLevel', String(m.effortLevel)]);
    }
    if (m.categorySlug) {
      triples.push([iri, 'kg:categorySlug', JSON.stringify(m.categorySlug)]);
    }

    for (const l of linksByMissionSlug.get(m.slug) ?? []) {
...
Note: `isWithinTTL(contentType, lastSeenAt)` is the 2-arg signature per `srv/lib/external-content-ttl.js` (NOT 3-arg with `endDate` — trials would use the 3-arg path, but Phase 4.3 ships missions-only).

- [ ] **Step 14: Wire it into the main projection loop**

Find Phase 4.2's `buildBlogPostTriples` call site (around line 309 per the file):
...
Add a parallel block immediately after the blog-post block. Pull the inputs from CDS in the same orchestration function:
...
const missions = await SELECT.from(DiscoveryMissions)
  .columns('slug', 'title', 'effortLevel', 'categorySlug', 'lastSeenAt');
const missionConceptLinks = await SELECT.from(DiscoveryMissionConceptLinks)
  .columns('mission.slug as missionSlug', 'concept.slug as conceptSlug')
  .where({ predicate: 'teaches' });

for (const t of buildDiscoveryMissionTriples({ missions, links: missionConceptLinks })) {
  triples.push(t);
}
```

(Adapt to the file's actual orchestration loop — match how it pulls `blogPosts` + `blogPostLinks`.)
...
Phase 4.2's `mergeOtherResources(journeyRows, blogRows)` has a TWO-POSITIONAL-ARG signature. Phase 4.3 must widen it to accept three arrays. The widening lands in Step 19 BEFORE the neighborhood-handler edit at Step 20.

- [ ] **Step 19: Widen `srv/lib/kg-neighborhood-merge.js` to accept variadic arrays**

```bash
grep -n "mergeOtherResources" srv/lib/kg-neighborhood-merge.js
```

Expected: `export function mergeOtherResources(journeyRows = [], blogRows = []) { ... }` at line ~31.

Change to variadic so any number of row arrays can be merged:
...
The variadic signature is BACKWARD-COMPATIBLE — existing callers passing two arrays still work; new callers can pass three or more.

- [ ] **Step 20: Run the existing Phase 4.2 helper test; expect PASS**

```bash
npx vitest run test/unit/srv/kg-neighborhood-merge
```

Expected: 4.2's existing tests still pass under the new variadic signature (positional calls survive).

- [ ] **Step 21: Extend `srv/knowledge-graph-service.js` neighborhood `otherResources`**

```bash
grep -n "otherResources\|blogOtherResources\|journeyOtherResources\|mergeOtherResources" srv/knowledge-graph-service.js
```

Phase 4.2 left the enrichment with TWO branches (`journeyOtherResources` + `blogOtherResources` merged via `mergeOtherResources(journeyOtherResources, blogOtherResources)`). Phase 4.3 adds a THIRD parallel branch.

Find the Phase 4.2 block (it lives inside a try/catch with graceful fallback). Add a parallel block AFTER the blog block, then update the `mergeOtherResources` call to pass the third array:
...
Update the existing `mergeOtherResources` call to pass the THIRD positional argument:

```javascript
// BEFORE (Phase 4.2 shape):
//   otherResources = mergeOtherResources(journeyOtherResources, blogOtherResources);
// AFTER (Phase 4.3 — third positional arg):
otherResources = mergeOtherResources(journeyOtherResources, blogOtherResources, missionOtherResources);
```

- [ ] **Step 22: Write a unit test `test/unit/srv/kg-neighborhood-missions.test.js`**
...
- [ ] **Step 23: Run; expect 4/4 PASS**

```bash
npx vitest run test/unit/srv/kg-neighborhood-missions.test.js
```

- [ ] **Step 24: Commit neighborhood enrichment**

```bash
git add srv/lib/kg-neighborhood-merge.js \
...
- kg-neighborhood-merge.js: mergeOtherResources widens from
  (journeyRows, blogRows) → variadic (...rowArrays). Backward-compatible:
  existing 2-arg call sites work unchanged.
- knowledge-graph-service.js: JS-side enrichment now UNIONs
  learning-journey + blog-post + discovery-mission overlap rows; sorts
  by overlapCount; caps top-5 TOTAL across types.
- Sidebar payload pre-resolves categoryLabel via the shared
  srv/lib/discovery-mission-categories.js helper at neighborhood-query
  time (unlike concept-page frontmatter which resolves at cron time).

4 unit tests on the merge helper cover 3-array variadic, type-discriminant
preservation, top-5 cap, AND backward-compat with 2-arg calls.
...
- [ ] **Step 25: Create `test/hybrid/discovery-missions-cron.test.js`**
