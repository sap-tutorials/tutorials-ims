...
git add srv/lib/help-docs/help-sap-com-fetcher.js \
        ...
        test/unit/srv/__fixtures__/help-sap-com-deliverable-metadata.json \
        test/unit/srv/__fixtures__/help-sap-com-page-content.json \
        test/unit/srv/__fixtures__/cap-cloud-sap-tree.json \
        test/unit/srv/__fixtures__/cap-cloud-sap-handlers.md \
        test/unit/srv/__fixtures__/ui5-topics-index.json \
        test/unit/srv/__fixtures__/ui5-topic-body.html
...
- help-sap-com-fetcher.js (~250 LoC): iterates HELP_SAP_COM_DELIVERABLES
  (~20 {product, deliverable} pairs seeded from HomepageShelves CSV per
  §4.2.2). Two-step walk per deliverable: /http.svc/deliverableMetadata
  (returns numeric deliverable.id + buildNo + landing filePath) then
  /http.svc/pagecontent (returns fullToc tree + per-topic HTML body).
  Walks fullToc; per topic emits one HelpDocRow. Filters title-empty
  + body<200-char. Per-deliverable 404 or non-OK status is survivable.
...
- ui5-sap-com-fetcher.js (~150 LoC): fetches /docs/topics/index.json
  (hierarchical {key,text,links} tree), walks it recursively; per topic
  fetches /docs/topics/<key>.html for the clean body HTML. Filters
  short-body + empty-title.
...
18 unit tests (6 + 5 + 6) with fixtures. Endpoints verified live
2026-07-01 via Playwright inspection of the SAP Vue SPAs — see
spec §4.2.2 + §4.2.4 + §4.2.4a for the internal-API tradeoff.
...
**Note:** the orchestrator returns `{ rows, perSource }`. The `perSource` field is what Task 2's cron logs in its per-source summary (spec §4.4 step 5). This test locks that shape in Task 1 so Task 2 doesn't retrofit it. Mock URL patterns match the VERIFIED endpoints from Steps 1-4: help.sap.com's `/http.svc/deliverableMetadata` + `/http.svc/pagecontent`, cap-js/docs's `/git/trees/`, ui5.sap.com's `/docs/topics/index.json` + `/docs/topics/<key>.html`.
...
**Approach:** uses `_setMockOrchestrator` to bypass the fetcher HTTP layer entirely and drive dedupe with hand-crafted `HelpDocRow[]` inputs. This isolates dedupe from any fetcher plumbing and makes each case's contentHash-collision setup explicit. Fetcher-level tests (Steps 18/23/28) already cover HTTP-layer concerns.
...
**Cross-check for Step 36:** the `computeContentHash` helper in `srv/lib/help-docs/index.js` (Step 36 below) must produce the hash from `title + description + product + section` — **NOT `source`**. If Step 36's implementation includes `source` in the hash material, cross-source rows can never collide and this test's first three cases will silently fail with "both rows survive" instead of the expected "cap wins over help". Verified alignment in Step 36's code sample.
...
- [ ] **Step 37: Run tests; expect 5/5 + 5/5 PASS.**
...
- `ui5-sap-com-fetcher.test.js` — 6 tests
- `help-docs-orchestrator.test.js` — 5 tests
- `help-docs-dedupe.test.js` — 5 tests
- `help-doc-slug-canonicalization.test.js` — 5 tests
- `kg-explore-data-iri-types.test.js` — 14 entries in expectedTypes (was 13)
