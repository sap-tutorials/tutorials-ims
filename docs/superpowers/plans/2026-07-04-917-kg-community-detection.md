...
- **Env flag:** `KG_COMMUNITIES_ENABLED` — `process.env.KG_COMMUNITIES_ENABLED === 'true'`. Default off. Gates admin-tile visibility ONLY; the nightly job always writes so a flip-on is instant.
