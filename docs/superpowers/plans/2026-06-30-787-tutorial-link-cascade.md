...
**Modify:**
- `db/schema.cds` — add 4 lines (comment + `conceptLinks` Composition) inside the existing `Tutorials` entity.
- `srv/lib/published-concepts-query.js` — modify the `teachesByConcept` block (lines ~63-66) to filter null-side rows before `.toLowerCase()`. ~6 line delta (1 line of filter + 4-line comment + reformatting the closure call across lines).
- `gen/db/last-dev/csn.json` + `gen/db/src/...` — auto-regenerated by `cds build --production` (DO NOT hand-edit). Commit them per project convention.
