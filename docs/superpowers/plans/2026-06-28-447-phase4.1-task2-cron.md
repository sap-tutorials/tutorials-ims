...
The above is a working scaffolding. The pipeline's `nearestConcepts` step uses a simplified "recent concepts" ordering rather than true embedding-similarity ranking (which would require a HANA SPARQL or vector-DB query). The simplification is called out in the comment and is acceptable for 4.1's MVP — admin can observe the quality and we can upgrade to similarity-ranking in a follow-up issue if extraction precision suffers.
