...
> **CORRECTION DISCOVERED DURING IMPLEMENTATION (2026-06-29):** This task is a **no-op** for this PR. The plan was written assuming `cds build --production` writes regenerated CSN + HDI artifacts to tracked paths under `db/last-dev/` and `db/src/`. It does not — per `.cdsrc.json`, the build target is `gen/` (gitignored). `db/last-dev/csn.json` is a **migration-baseline subset** (~42 definitions) that does not include any of the homepage entities (`HomepageShelves` was added for #639 the same way — without updating `db/last-dev/csn.json`). The `check-cds-build-staging` CI guard runs `cds build` + `git diff --exit-code` on `db/last-dev/` and `db/src/`; with zero diff on a pure-additive schema change, the guard passes. **No commit is made for this task; skip directly to Task 6.** The HDI artifacts are correctly produced at `mbt build` / `cf deploy` time.
