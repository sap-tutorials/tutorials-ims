...
**Additional design note (2026-07-09, after security-review pass caught open-redirect hazards):** RFC 8252 §7.3 requires loopback redirect URIs to be `http://127.0.0.1:*` / `http://localhost:*` with a *fixed callback path*, not open path. And custom-scheme wildcards (`mcp://*`) are OAuth open-redirect hazards on any OS that resolves that scheme to a running app. Corrected redirect-URI additions: `http://127.0.0.1:*/callback`, `http://localhost:*/callback`, `https://developers.sap.com/callback`. NO `mcp://*` — the loopback pair covers Claude Desktop / Claude Code / mcp-remote; if a native scheme is ever needed later, register a fully-qualified reverse-DNS scheme like `com.sap.tutorials.mcp:/oauth/callback` without wildcards.
