...
**Goal:** Close #450 by adding `TutorialMeta.firstNotificationDate` + `MyTutorialsView.outdated` to the already-working author-review nag system, tightening the stale threshold 180 → 90 days across 4 call sites, and clearing the new field in all 3 reset code paths.
...
- No data migration. Rows already at notification level 1-3 stay with `firstNotificationDate = NULL` forever.
...
| File | Change |
| --- | --- |
| `db/schema.cds` | Add `firstNotificationDate : Timestamp;` to `TutorialMeta` entity (Task 1) |
| `db/views.cds` | Extend `MyTutorialsView` select-list with `firstNotificationDate` + `outdated : Boolean` (Task 2) |
| `srv/lib/contributor-notifications.js` | `STALE_DAYS_DEFAULT` 180 → 90 (Task 5); `markNotificationSent` sets `firstNotificationDate` on first nag (Task 3) |
| `srv/lib/tutorial-review.js` | `reviewTutorial` `.set({...})` adds `firstNotificationDate: null` (Task 4) |
| `srv/jobs/scheduler.js` | Line 135 `computeStaleNotifications(180)` → `(90)` (Task 5) |
| `srv/admin-service.js` | Line 794 `computeStaleNotifications(180)` → `(90)` (Task 5); line 356 `before('UPDATE','TutorialMeta')` hook adds `req.data.firstNotificationDate = null` (Task 6) |
| `scripts/seed-tutorial-meta.js` | Lines 50 + 60 `180` → `90` + extract `STALE_THRESHOLD_DAYS` constant (Task 6) |
| `test/unit/lib/tutorial-review.test.js` | Extend existing `reviewTutorial` test with `firstNotificationDate: null` assertion (Task 4) |
...
- `gen/db/src/gen/com.sap.developers.ims.TutorialMeta.hdbmigrationtable` — new `migration=3` block with `ALTER TABLE ... ADD (firstNotificationDate TIMESTAMP)` (auto-generated; do NOT hand-edit; verify in Task 1.5 + Task 8)
...
## Task 1: Schema — add `firstNotificationDate` column to `TutorialMeta`
...
- [ ] **Step 1.4: Verify the column is in the CSN**
  ...
  print('firstNotificationDate:', m.get('elements', {}).get('firstNotificationDate', 'MISSING'))
  "
  ```

  Expected: `firstNotificationDate: {'type': 'cds.Timestamp'}` (or similar; key is that it's NOT `MISSING`).
...
- [ ] **Step 1.7: Commit**
  ...
  git commit -m "feat(schema): add TutorialMeta.firstNotificationDate (#450)
  ...
  stay with firstNotificationDate=NULL forever (no data migration per
  spec).
...
## Task 2: View — extend `MyTutorialsView` with `firstNotificationDate` + `outdated`
...
- [ ] **Step 2.3: Verify view compiles**
  ...
  Expected: 2× `OK`. If a compile fails with "element not found: firstNotificationDate", Task 1 didn't add the column correctly.
...
- [ ] **Step 2.4: Verify the calc field landed in CSN**
  ...
  print('firstNotificationDate:', v.get('elements', {}).get('firstNotificationDate', 'MISSING'))
  "
  ```
...
- [ ] **Step 2.7: Commit**
  ...
  git commit -m "feat(view): expose firstNotificationDate + outdated on MyTutorialsView (#450)
  ...
  firstNotificationDate is the new TutorialMeta column from Task 1;
  pass-through so Sage can show '1st nag sent on YYYY-MM-DD' context."
  ```

---

## Task 3: Lib — `markNotificationSent` sets `firstNotificationDate` on first nag (TDD)
...
- [ ] **Step 3.3: Run the new tests — expect FAIL**
  ...
  Expected: the 2 new tests FAIL because `firstNotificationDate` is never written. Specifically:
  - Test 1: `updated.firstNotificationDate` is `null` or `undefined` (not truthy).
  - Test 2: `updated.firstNotificationDate` is unchanged but `markNotificationSent` doesn't read or write it (passes accidentally? — actually depends; if the column exists as nullable, the test should still verify it stays equal to the originally-inserted value, which it WILL be since we don't touch it. Test 2 may pass at red. Test 1 is the critical failing test.)
...
- [ ] **Step 3.8: Commit**
  ...
  git commit -m "feat(lib): markNotificationSent sets firstNotificationDate on first nag (#450)

  Spread-conditional pattern: when meta.notificationNumber=0 BEFORE the
  increment, set firstNotificationDate=now alongside the existing
  lastNotificationDate update. On nags 2/3/4, firstNotificationDate is
  ...
  describe('markNotificationSent firstNotificationDate tracking'):
  - sets firstNotificationDate on the first nag
  - does NOT overwrite on subsequent nags"
  ```

---

## Task 4: Lib — `reviewTutorial` clears `firstNotificationDate` (TDD)
...
- [ ] **Step 4.2: Extend the existing test fixture + assertion (red phase)**
  ...
  The existing `beforeEach` (lines 13-24) seeds a `TutorialMeta` row with `notificationNumber: 5, lastNotificationDate: '2024-01-01T00:00:00Z'` (but no `firstNotificationDate`). We need to:

  1. Add `firstNotificationDate: '2024-01-01T00:00:00Z'` to the existing INSERT so the test has something to clear.
  2. Add an assertion that after `reviewTutorial`, the persisted row has `firstNotificationDate: null`.
...
- [ ] **Step 4.3: Run the test — expect FAIL**
  ...
  Expected: the test FAILS on the new assertion `expect(persisted.firstNotificationDate).toBeNull()` — the current `reviewTutorial` doesn't clear that field, so it stays at `'2024-01-01T00:00:00Z'`.
...
- [ ] **Step 4.8: Commit**
  ...
  git commit -m "feat(lib): reviewTutorial clears firstNotificationDate (#450)

  reviewTutorial now resets all 4 review-state fields atomically:
  reviewedDate (set to now), notificationNumber (0),
  lastNotificationDate (null), firstNotificationDate (null).

  Test extended: now also asserts the persisted row has both
  lastNotificationDate=null and firstNotificationDate=null."
  ```
...
- [ ] **Step 5.2: Add 2 new edge-case tests inside the same describe block**
  ...
  Use Edit on `test/lib/contributor-notifications.test.js`. Anchor on the closing `});` of the `describe('markNotificationSent firstNotificationDate tracking', ...)` block (added in Task 3).
...
- [ ] **Step 6.9: Commit**
  ...
  git commit -m "feat(admin+seed): symmetric firstNotificationDate clear + 90-day seeder (#450)
...
- [ ] **Step 7.8: Commit**
  ...
  1. beforeAll fixture amended: TutorialMeta gets ownerEmail (the
     ...
     fixture seeded with firstNotificationDate to verify it clears.
...
- [ ] **Step 8.4: Verify the 4 threshold sites + the 3 reset paths land together**
  ...
  echo "--- markNotificationSent firstNotificationDate set ---"
  grep -n "firstNotificationDate: now\|isFirstNag" srv/lib/contributor-notifications.js
  echo "--- reviewTutorial firstNotificationDate clear ---"
  grep -n "firstNotificationDate: null" srv/lib/tutorial-review.js srv/admin-service.js
  ```
...
- [ ] **Step 8.7: Open the PR via `gh pr create`**
  ...
  gh pr create --base main --head worktree-issue-450-author-review-lifecycle \
    --title "feat(author-review): close #450 long-tail (firstNotificationDate + outdated + 90d threshold)" \
    --body-file - <<'BODY'
  Closes #450.
  ...
  **Schema + view (Tasks 1-2):**
  - `TutorialMeta.firstNotificationDate : Timestamp` (nullable; populated on
    first nag, cleared on review).
  - `MyTutorialsView` exposes `firstNotificationDate` + `outdated : Boolean`
    (derived from `notificationNumber >= 4`).

  **Lib changes (Tasks 3-4):**
  - `markNotificationSent`: spread-conditional set of `firstNotificationDate`
  ...
  **Tests (Tasks 3-7):**
  - `test/lib/contributor-notifications.test.js` extended: 6 tests total
    (2 updated for new threshold + 2 firstNotificationDate + 2 filter-edge).
  - `test/unit/lib/tutorial-review.test.js` extended: assertion that
    `firstNotificationDate: null` post-reset.
...
- **HANA case-sensitivity**: HANA uppercases identifiers; CDS quoted-lowercase identifiers fail under HDI deploy. The new `firstNotificationDate` will be `FIRSTNOTIFICATIONAT` in `.hdbmigrationtable`. Memory `[feedback_hana_raw_sql_uppercase]`.
