...
7. **Extract fenced code blocks** from step text using `String.prototype.matchAll` against a regex that matches triple-backtick blocks. In the JS source, write the regex with bare backticks inside the literal (NOT escaped — markdown forces escaping in this plan, but in `.js` files they are plain backticks): `const FENCED = /```([a-z0-9]*)\n([\s\S]*?)```/gi;`. Note: use `matchAll` — NOT the regex method whose name starts with "ex" (the security hook flags that name as a child_process false positive). Join blocks with blank lines. Empty string if no fenced blocks.
