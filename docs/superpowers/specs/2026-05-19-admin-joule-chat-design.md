...
```json
{
  ...
  "sections": [
    {
      "path": "docs/developers/architecture/build.md",
...
Admin: "How does delta publishing work?"
  ...
  → Runner returns 3 sections from docs/developers/architecture/build.md
  → SSE { type: 'doc-citations', items: [...] }
  → LLM follow-up turn produces prose answer citing docs/developers/architecture/build.md
  → SSE { type: 'done' }
```
...
- Ask "How does delta publishing work?" → assistant cites `docs/developers/architecture/build.md`.
...
3. `searchAdminDocs` returns relevant sections from `docs/developers/architecture/build.md` for "delta publishing" query.
