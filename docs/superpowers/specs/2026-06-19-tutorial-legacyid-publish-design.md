...
For each Tutorials row where legacyId IS NULL:
  ...
  snapshot the before-state of: Tutorials row, matching TaskRecords, matching CompletionPathItems
  UPDATE Tutorials SET legacyId = newId WHERE ID = <row.ID> AND legacyId IS NULL
  UPDATE TaskRecords SET taskLegacyId = newId
    WHERE taskLegacyId IS NULL
      AND taskType = 'TUTORIAL'
      AND LOWER(taskSlug) = LOWER(<row.slug>)
...
**Snapshot:** JSONL file at `.migration-data/tutorial-legacyid-repair-backup-<ISO>.jsonl`. One line per touched row (Tutorials + each updated TaskRecord + each updated CompletionPathItem). Mirrors `dedupe-tutorial-meta.cjs`'s convention.

**Snapshot guards in the repair logic:**
- If a TaskRecord already has a non-NULL `taskLegacyId` that disagrees with the new one, log a warning and skip — that row was likely written before the bug existed (different code path) and shouldn't be silently overwritten.
- Same for CompletionPathItems.
- The `WHERE taskLegacyId IS NULL` clause already encodes this; the warning surfaces if the SELECT count exceeds the UPDATE count.
...
2. Inserts matching TaskRecords + CompletionPathItems with `taskLegacyId: null`.
3. Runs the repair logic against just those `__TEST__` slugs (not the whole DB).
4. Asserts: Tutorials.legacyId now non-null; TaskRecords.taskLegacyId now non-null; CompletionPathItems.taskLegacyId now non-null; all three carry the same value.
...
| Repair row's slug matches >1 Tutorials row (mixed-case duplicate) | Should not happen post-PR #386 (`@assert.unique.slug`). If it does, repair updates the Tutorials row in question by ID, but the LOWER() match for TaskRecords/CompletionPathItems may pick up rows linked to the OTHER duplicate. | Hybrid test asserts the post-#386 invariant holds. If a future regression introduces dups, the repair logs the duplicate count and skips (`--commit` no-op for that slug). |
...
| TaskRecord has `taskLegacyId: <some-other-value>` (not NULL but wrong) | The `WHERE taskLegacyId IS NULL` clause leaves it alone. Logged as "skipped — already has taskLegacyId=X". | Manual triage if the count is non-zero — likely indicates a third bug elsewhere. |
...
2. **Hybrid test 2 (repair script)**: a manually-NULLed Tutorials row + matching TaskRecord + matching CompletionPathItem all heal to the same legacyId after running the repair logic.
