...
- **Which pages:** all content pages, in one change —
  ...
  - missions + groups (CAP SSR via `composeShell`)
  - topic pages (CAP SSR)
  - **excluded:** `/browse/`, verb lanes, sitemaps, homepage — these are
    aggregate/navigation pages with no single product-tag identity.
...
### Component 5 — SSR emit (`composeShell`, `catalog-data.js`, `topic-list-page.js`)
...
`smTechIds` param), so we **insert** rather than replace:
...
- In `composeShell({before, after}, bodyHtml, meta)`: when
  ...
  immediately **after** the description-meta rewrite anchor. Guard against
  double-insert (idempotent `.replace` of any pre-existing sm_tech_ids meta
  first, matching the module's defensive style). `content` value is escaped
  via the existing `escapeAttr`.
- `catalog-data.js` `loadMissionContext`/`loadGroupContext`: after loading the
  entity, resolve its product tags → `MissionTags`/`GroupTags` → `tag_ID` →
  `Tags(isActualTag=true, semaphoreId≠null)`; attach `smTechIds: string[]`.
  Read `Tags.semaphoreId` directly (SSR has the tag rows — no mdFormat round
  trip needed here). `catalog-renderer.js` copies it into `pageMeta`.
- `topic-list-page.js`: a topic *is* a tag; emit that tag's own `semaphoreId`
  when it is an `isActualTag` product tag (else no meta).

Fail-open everywhere: any resolution error → empty list → no meta tag; never
throw into the render/serve path.
...
| Mission | runtime: `MissionTags` → `Tags.semaphoreId` | `composeShell` insert |
| Group | runtime: `GroupTags` → `Tags.semaphoreId` | `composeShell` insert |
| Topic | runtime: topic tag's own `semaphoreId` | topic SSR head |
| browse/verb/sitemap/home | — (excluded) | none |
...
- **Smoke (post-deploy):** `curl` a known tutorial + a known mission and grep
  the served HTML for `name="sm_tech_ids"` with a locale-prefixed content.
...
- `srv/lib/chrome-shell.js` — `composeShell` insertion
