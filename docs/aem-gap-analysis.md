...
**Residual cleanup (not blocking cutover):**

- `hugo/content/tutorials/test-tutorial.md` is the only tracked tutorial file (the rest are gitignored, generated by `fetch-tutorials`); it leaks into the public sitemap. Either delete the fixture or add `private: true` to its frontmatter.
- `/scanner-vue/`, `/event-display/`, `/app-space/` are internal/auth-gated sections currently included. Add `private: true` to their `_index.md` frontmatter to exclude (the existing layout filter already drops `Params.private` pages).
