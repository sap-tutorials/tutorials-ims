...
Export artifacts land in `.migration-data/` (gitignored). The same directory holds `slug-mapping.json`, which is **the canonical slug source for fresh DB deploys** — `scripts/setup-dev-data.cjs` consumes it via `npx cds bind --exec` to assign slugs to records that lack them. Per [CLAUDE.md](https://github.com/sap-tutorials/tutorials-ims/blob/main/CLAUDE.md), the legacyId match is best-effort; a slug just needs to exist for `/build/catalog` to surface text slugs instead of numeric IDs.
