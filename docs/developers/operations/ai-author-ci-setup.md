# AI authoring CI setup (#208 spike)

> **Status:** required setup before running `rebuild-content.yml` or
> `rebuild-content-qa.yml` with `ai-author-enabled=true`. See
> [issue #208](https://github.com/sap-tutorials/tutorials-ims/issues/208) for the spike's vision and
> [issue #275](https://github.com/sap-tutorials/tutorials-ims/issues/275) for the graduation gate.

## Why this exists

`AI_AUTHOR_ENABLED` is read by `scripts/fetch-tutorials.ts` — a
...
3. Add `[AUTOAUTHOR_ALL]` (or per-step `[AUTOAUTHOR_N]`) on its own line.
   ...
   that step.
4. Open a PR. Once merged, the next CI rebuild with
   `ai-author-enabled=true` will generate AI questions for every step
   that doesn't have hand-authored content.
...
When `ai-author-enabled=true` and the rebuild succeeds:
...
The flag is per-run (workflow_dispatch input), so "off" is the default.
Don't set it `true` for routine rebuilds — burning $1–$2 of LLM calls
per push doesn't make sense until the spike graduates and AI questions
are part of the production pipeline.

After issue #275's graduation decision lands:

- **Graduate:** delete the input + always-on the AI-quiz step.
- **Iterate:** keep the input around for v2-prompt experiments; default
  stays `false`.
- **Shelve:** remove the workflow inputs + the `Set up VCAP_SERVICES`
  step + the AI-related env vars from the Fetch tutorials step.

## Common failure modes

| Symptom | Likely cause |
|---|---|
| `::error::ai-author-enabled=true requires AI_AUTHOR_AICORE_SERVICE_KEY secret` | Step 3 wasn't done. |
