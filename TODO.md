...
- [ ] **Admin UI Tags: missing columns** — The Tags list report is missing the "MD Format" and "Full Path" columns. These fields don't exist in the CDS schema (`db/schema.cds` only has `name` and `legacyId`) — implementing this requires adding new entity fields first, which is a schema change task.
