...
export const RESOURCE_TYPE_CONFIG = Object.freeze([
  ...
  },
  {
    type: 'teched-session',
    icon: '🎤',
    singular: 'SAP TechEd session',
    plural: 'SAP TechEd sessions',
    priority: 100,
    metaTemplate: 'Speaker · Date · Room',
    renderMeta(r) {
      // #2312: speakerNames + scheduledStart + room from TechEdSessions (the
      // neighborhood loader composes speakerNames from the session↔speaker
      // junction). The 🎥 icon is appended when a YouTube recording exists so
      // the sidebar surfaces watchable sessions without extra UI chrome.
      const row = r || {};
      const parts = [];
      if (row.speakerNames) parts.push(`by ${row.speakerNames}`);
      if (row.scheduledStart) {
