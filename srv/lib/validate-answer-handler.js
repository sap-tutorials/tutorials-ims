// srv/lib/validate-answer-handler.js
// Express handler factory for POST /api/validate-answer.
// Rate-limits per user (30/hour) and per (user, slug, step) (5/5 min).
// LLM caller and question-loader are injected at the call site (srv/server.js)
// so unit tests run without network or HANA, and the closure can never
// capture a stale null from a module-level variable.
//
// Mirrors srv/lib/code-check-handler.js (PR #205) — same windows, same
...
/**
 * Create the POST /api/validate-answer Express handler.
 *
 ...
 * @param {Function} deps.loadQuestion - Question loader: (slug, step, qid) → row|null.
 * @returns {import('express').RequestHandler}
 */
export function makeValidateAnswerHandler(deps = {}) {
  const { callModel, loadQuestion } = deps;
  ...
  return async function validateAnswerHandler(req, res) {
    ...
    // Transient upstream errors (and disabled) do NOT count against the user's
    // quota — the system isn't on, or our flake shouldn't punish learners.
