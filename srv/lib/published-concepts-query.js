...
export async function buildConceptsPayload(db) {
  ...
  // Steps 2–5h below are independent: each reads its own link table and
  // filters by idsSet (computed above). None depends on another's result,
  // so issue them concurrently rather than serially — on HANA this collapses
  // ~11 sequential round-trips into a single wave, which was pushing
  // /build/concepts past the 30s smoke-gate timeout (~50–60s cold). Each
  // block owns its read + Node-side grouping and returns the finished
  // per-concept map; step 6 stitches the resolved maps unchanged.
  const [
    teachesByConcept,
    { requiresByConcept, relatedToByConcept, requiredByConcept },
    learningJourneysByConcept,
    blogPostsByConcept,
    discoveryMissionsByConcept,
    videosByConcept,
    apiDocsByConcept,
    samplesByConcept,
    helpDocsByConcept,
    communityEventsByConcept,
  ] = await Promise.all([
    ...
    // 3 + 4. Outgoing edges (requires + relatedTo) and incoming "requires"
    // edges (so the page can show "required by"). Two reads, run in parallel,
    // that jointly produce the three edge maps.
    (async () => {
      const [outgoingRows, incomingRows] = await Promise.all([
        db.run(
          SELECT.from(ConceptEdges)
            ...
            .where({ status: 'ACTIVE' })
        ).then(rows => rows.filter(r => idsSet.has(r.source_ID))),
        db.run(
          SELECT.from(ConceptEdges)
            .columns(
              'target_ID', 'predicate',
        ...
        ).then(rows => rows.filter(r => idsSet.has(r.target_ID))),
      ]);
      ...
      return { requiresByConcept, relatedToByConcept, requiredByConcept };
    })(),
    ...
    (async () => {
      if (ids.length === 0) return {};
      const discoveryMissionsByConcept = {};
      ...
      return discoveryMissionsByConcept;
    })(),
    ...
    (async () => {
      if (ids.length === 0) return {};
      const communityEventsByConcept = {};
      ...
      return communityEventsByConcept;
    })(),
  ]);
