...
//
// Uses the SAME IN_SCOPE_PAGES list and extForMime helper from
// srv/lib/page-key-map.js (single source of truth).
// page-key-map.js is ESM, so we use dynamic import().
//
...
//   0  some non-homepage pages missing from hugo/public (warns; partial ok).
//   1  page-index (homepage) snapshot missing — hard build gate.
//   1  unexpected error.
...
(async () => {
  try {
    const { IN_SCOPE_PAGES, extForMime } = await import('../srv/lib/page-key-map.js');

    mkdirSync(FALLBACK_DIR, { recursive: true });

    let copied = 0;
    let missing = 0;
    let pageIndexCopied = false;
    for (const p of IN_SCOPE_PAGES) {
      ...
      if (p.key === 'page-index') pageIndexCopied = true;
      copied++;
    }
    ...
    if (!pageIndexCopied) {
      console.error('[build-page-fallback] ERROR: homepage fallback snapshot page-index.html missing — refusing to build; a HANA hiccup on / must not be an outage');
      process.exit(1);
    }
  } catch (err) {
